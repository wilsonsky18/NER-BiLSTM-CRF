...
def load_sentences(path, lower, zeros):
    ...
    :return sentences = [[sentence_1],[sentence_2]...]
    sentence = [[word_1,tag_1],[word_2,tag_2]...]
    """
    sentences = []
    sentence = []
    num = 0
...
def prepare_dataset(sentences, char_to_id, tag_to_id, lower=False, train=True):
    ...
    for s in sentences:
        ...
        chars = [char_to_id[f(w) if f(w) in char_to_id else '<UNK>']
                 for w in string] # list contains each lowcase word in sentence
        segs = get_seg_features("".join(string)) # list contains word groups cut by jieba
        if train:
...
if __name__ == "__main__":
    #test_sentences = load_sentences(os.path.join("data", "example.test"), True, False)
