...
class Model(object):
    def __init__(self, config):
        ...
        self.num_segs = 4
        self.use_start_end_crf = config["use_start_end_crf"]
    ...
    def biLSTM_layer(self, lstm_inputs, lstm_dim, lengths, name=None):
        ...
        """
        lstm_fw_cell = tf.nn.rnn_cell.BasicLSTMCell(lstm_dim, forget_bias=1.0, name='lstm_fw_cell')
        lstm_bw_cell = tf.nn.rnn_cell.BasicLSTMCell(lstm_dim, forget_bias=1.0, name='lstm_bw_cell')
    ...
    def loss_layer(self, project_logits, lengths, name=None):
        ...
        with tf.variable_scope("crf_loss" if not name else name):
            if self.use_start_end_crf:
                ...
                # shape = (batch_size, 1 , num_tags+2)
                start_logits = tf.concat(
                    [small * tf.ones(shape=[self.batch_size, 1, self.num_tags]),
                     tf.zeros(shape=[self.batch_size, 1, 1]), small * tf.ones(shape=[self.batch_size, 1, 1])], axis=-1)
                ...
                # shape = (batch_size, nums_steps , num_tags+2)
                logits = tf.concat([project_logits, pad_logits], axis=-1)
                # shape = (batch_size, nums_steps + 1 , num_tags + 1)
            ...
            log_likelihood, self.trans = crf_log_likelihood(
                inputs=logits,
                tag_indices=targets,  # true label
                transition_params=self.trans,  # transition matrix
                sequence_lengths=lengths  # sequence length
            )
